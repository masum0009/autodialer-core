...
static void internal_register_handler(struct sipsess_sock *sock, const struct sip_msg *msg)
{
	struct sip *sip = sock->sip;
...
static bool request_handler(const struct sip_msg *msg, void *arg)
{
	...
	if (!pl_strcmp(&msg->met, "INVITE")) {
		...
		if(sock->regh != NULL)	sock->regh(sock, msg);
    else internal_register_handler(sock, msg);
		return true;
	}
